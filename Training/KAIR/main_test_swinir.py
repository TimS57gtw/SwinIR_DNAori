...
from Training.KAIR.models.network_swinir import SwinIR as net
from utils import util_calculate_psnr_ssim as util


def main(model, infld, outfld):
    ...
    jpg = 40
    lqf = infld
    # lqf = "testsets/classic5"
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, default=task, help='classical_sr, lightweight_sr, real_sr, '
                                                                     'gray_dn, color_dn, jpeg_car, color_jpeg_car')
    parser.add_argument('--scale', type=int, default=scale, help='scale factor: 1, 2, 3, 4, 8') # 1 for dn and jpeg car
    parser.add_argument('--noise', type=int, default=noise, help='noise level: 15, 25, 50')
    parser.add_argument('--jpeg', type=int, default=jpg, help='scale factor: 10, 20, 30, 40')
    ...
    parser.add_argument('--model_path', type=str,
                        default=model)
    parser.add_argument('--folder_lq', type=str, default=lqf, help='input low-quality test image folder')
...
def define_model(args):
    ...
    # 003 real-world image sr
        ...
        if not args.large_model:
            ...
            model = net(upscale=1, in_chans=1, img_size=32, window_size=8,
                        img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                        mlp_ratio=2, upsampler=None, resi_connection='1conv')
            print("model")
        else:
            # larger model size; use '3conv' to save parameters and memory; use ema for GAN training
            model = net(upscale=args.scale, in_chans=1, img_size=64, window_size=8,
    ...
    pretrained_model = torch.load(args.model_path, encoding='latin1')
