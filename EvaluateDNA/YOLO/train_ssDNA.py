...
def parse_opt(known=False):
    data = 'ssDNA_Mix_4k_256p_pt2.yaml'
    try:
        idx = len(os.listdir(data.split(".")[0]))
    except FileNotFoundError:
        idx = 0
    proj = data.split(".")[0]
    comment="Autoanch"
    name = proj + str(idx)+comment
    print("Starting", name)
    freeze = 0
    epochs = 1600
    img_size = 256
    ...
    parser.add_argument('--optimizer', type=str, choices=['SGD', 'Adam', 'AdamW'], default=optim, help='optimizer')
    parser.add_argument('--sync-bn', action='store_true', help='use SyncBatchNorm, only available in DDP mode')
    parser.add_argument('--workers', type=int, default=2, help='max dataloader workers (per RANK in DDP mode)')
    parser.add_argument('--project', default=proj, help='save to project/name')
    ...
    parser.add_argument('--freeze', nargs='+', type=int, default=[freeze], help='Freeze layers: backbone=10, first3=0 1 2')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            ...
            'anchor_t': (1, 0.5, 8.0),  # anchor-multiple threshold ---> (1, 2.0, 8.0)
            'anchors': (1, 0.3, 10.0),  # anchors per output grid (0 to ignore) ---> (2, 2, 10)
